![XRON](https://img.shields.io/badge/XRON-Lossless%20Compression-ff6600?style=flat-square&logo=data:image/svg+xml;base64,)

# PR: Add XRON Output Parser & Document Compressor for LangChain

## Summary

This PR adds first-class XRON support to LangChain via two new components:
...
**Zero hallucination guarantee** — XRON is an algorithmic encoder, not an LLM summariser. It never drops, approximates, or fabricates data. `XRON.parse(XRON.stringify(data))` deep-equals the original, every time.
...
```typescript
import { XRONOutputParser, XRONDocumentCompressor } from 'xron-langchain';
...
- 5 unit tests covering parse, code-fence stripping, format instructions, and document compression round-trips
...
- **Live Playground**: [xron.vercel.app](https://xron.vercel.app)
