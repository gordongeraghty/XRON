...
## 2024-05-15 - Optimize String Concatenation Pattern
**Learning:** When parsing or splitting strings based on delimiters, character-by-character string concatenation (`current += ch`) in tight loops creates significant memory allocations and garbage collection pressure in JS engines. Repeated concatenation can be a silent performance killer in heavily used parsing pipelines.
**Action:** Track the start index (`start = i + 1`) and use a single `String.prototype.slice(start, i)` operation. For operations like unescaping, establish an early-return fast path (e.g., `if (!str.includes('\\')) return str`) using native string search before falling back to manual slicing.
