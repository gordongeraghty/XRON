...
## 2024-10-25 - Avoid spreading strings to arrays for reverse string matching
**Learning:** In `longestCommonSuffix`, spreading strings into character arrays (`[...str]`), reversing them, and joining them (`.reverse().join('')`) incurs severe performance and memory overhead for large arrays. Benchmarks show replacing this with an iterative index-based code-unit comparison is over 20x faster.
**Action:** When finding common prefixes or suffixes among strings, always iteratively compare code units using index access (e.g., `str[i]`) and adjust for UTF-16 surrogate boundaries manually, rather than relying on full string reverse allocations.
