...
export function splitRow(row: string): string[] {
  ...
  // Match original behavior: only push if there's something to push
  // For standard CSV like split `a, b, ` the last piece is empty but might have been included.
  // Wait, let's look at the original behavior:
