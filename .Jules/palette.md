...
## 2024-05-11 - Add Empty States for Multi-Selects
**Learning:** In multi-select filter components (e.g., format toggles), avoiding leaving a completely blank screen when all options are disabled prevents a confusing UX. Implementing a visually structured empty state that includes a clear call-to-action button (like 'Reset to defaults') provides an immediate path to recovery.
**Action:** Always implement a visually structured empty state with a clear recovery path when all options in a multi-select are disabled.
