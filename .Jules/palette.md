...
## 2024-06-03 - Empty State Call-to-Action for Multi-selects
**Learning:** When all options in a multi-select component (like format toggles) are disabled, leaving a blank or text-only empty state provides a poor user experience with no clear recovery path.
**Action:** Always implement a visually structured empty state that includes a clear call-to-action button (e.g., "Reset to defaults") to give users an immediate path to restore functionality.
