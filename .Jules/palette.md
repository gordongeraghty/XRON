...
## 2026-07-19 - Empty State Recovery Paths
**Learning:** In multi-select filter components (like format toggles), a completely blank screen when all options are disabled causes confusion and traps the user without clear next steps.
**Action:** Always implement a visually structured empty state that includes a clear call-to-action button (like 'Reset to defaults') to provide an immediate path to recovery and improve UX.
