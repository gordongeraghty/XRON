...
## 2026-07-10 - Empty state call-to-action for format multi-select
**Learning:** In multi-select filter components (e.g., format toggles), leaving a completely blank screen when all options are disabled leads to poor UX. A visually structured empty state provides immediate recovery paths.
