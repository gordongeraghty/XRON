...
## 2026-06-22 - Empty States for Multi-select Components
**Learning:** In multi-select filter components, avoiding a completely blank screen when all options are disabled is crucial. A structurally distinct empty state with a clear call-to-action (like 'Reset to defaults') provides an immediate path to recovery.
**Action:** Always implement a visually structured empty state with a reset button (and focus-visible styles) rather than simple text.
